...
- Network: both `envoy-internal` (172.19.32.1) and `envoy-external` (172.19.32.2, via a dedicated `cloudflared` Cloudflare Tunnel, external hostname `min-ext.rtrox.io`) are deployed. Mini's external tunnel only routes `*.mini.rtrox.io` (not the bare `*.rtrox.io` domain, which chongus's own tunnel already owns) to avoid DNS/tunnel-routing collisions between clusters sharing the `rtrox.io` zone.
